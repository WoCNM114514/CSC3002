...
void PriorityQueue<ValueType>::enqueue(ValueType value, double priority) {
   ...
   {  // if the next temp exists
      // current temp is more prior than the enqueue one 
      while (temp->link != NULL && temp->link->priority <= priority)
      {
         temp = temp->link;
      }
      // if the enqueue is more prior than the first element, change head to temp
      if (temp->priority > cp->priority)
      {
         cp->link = head;
         cp = head;
      }
      // else if the temp is not last
      // put enqueue to current position
      else if (temp->link != NULL)
      {
         cp->link = temp->link;
         temp->link = cp;
      }
      // else if temp is the last
      // put the enqueue one into last position
      else if (temp->link == NULL)
      {
         temp->link = cp;
      }
